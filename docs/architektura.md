...
Diagnostyka uruchamia tylko `ffmpeg -version` oraz `ffprobe -version`, zbiera wersje i czytelne błędy. Obok niej istnieje moduł budujący komendę wyodrębniania audio do WAV, ale ta ścieżka nie jest jeszcze podpięta do webowego UI. FFmpeg nie analizuje jeszcze audio, nie wykrywa ciszy i nie wykonuje montażu.
